...
def federated_learning():
    ...
    sf = 3
    lam = 330
    m_prime = nextprime(1000*pow(10,10))
    m = pow(m_prime,lam)
    d = 30
    

    X_full = [vector(RR, [0.4, -0.6]) for _ in range(150)] + [vector(RR, [-0.7, 0.3]) for _ in range(150)]
    y_full = [1]*150 + [0]*150
    X_train, X_test, y_train, y_test = train_test_split(X_full, y_full, test_size=0.2r, random_state=42)
